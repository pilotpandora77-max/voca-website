...
export default function BookReader() {
  ...
  function goToPage(n) {
    ...
    setPage(clamped);
    setAiReply(null); setSelection(null);
    clearTimeout(saveTimer.current);
    saveTimer.current = setTimeout(async () => {
      try {
        const { data } = await api.put(`/api/books/${id}/progress`, { page: clamped });
  ...
  async function toggleBookmark() {
    try {
      const { data } = await api.post(`/api/books/${id}/bookmark`, { page });
  ...
  async function explainSelection() {
    ...
    setAiLoading(true);
    try {
      const { data } = await api.post('/api/books/explain', { text: selection.text });
  ...
  return (
    <div style={{ padding: '16px 28px 40px', maxWidth: 900, margin: '0 auto' }} onMouseUp={onMouseUp}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
        <Link href={`/books/${id}`} style={{ color: 'var(--text-sub)', textDecoration: 'none', fontWeight: 700, fontSize: 14 }}>← Буцах</Link>
