...
function LoginInner() {
  ...
  const [googleBtnWidth, setGoogleBtnWidth] = useState(560);
  ...
  // Google-ийн жинхэнэ товч (үл үзэгдэх, дээрээ хиймэл товч давхарлагдсан) яг
  // харагдаж буй хайрцгийн өргөнтэй тааруулж байх ёстой — эс тэгвэл дарах цэг
  // хиймэл товчны хилээс гарч, товч "ажиллахгүй" мэт санагдана (жижиг дэлгэцэд
  // ялангуяа гарч байсан асуудал, stage scale-ийг хассанаас болов).
  useEffect(() => {
    if (!googleBtnRef.current) return;
    const el = googleBtnRef.current;
    const measure = () => {
      const w = el.getBoundingClientRect().width;
      if (w) setGoogleBtnWidth(Math.round(w));
    };
    const ro = new ResizeObserver(measure);
    ro.observe(el);
    // Фонт ачаалагдсаны дараа layout бага зэрэг өөрчлөгдөж болох тул
    // (ResizeObserver үргэлж дараагийн жинхэнэ хэмжээгээр дахин duudагддаггүй
    // тохиолдол ажиглагдсан) — фонт бэлэн болмогц нэг удаа дахин хэмжинэ.
  ...
  // Далд давхарлагдсан Google iframe товч зарим mobile хөтөч дээр tap-д
  // хариу өгөхгүй байх тохиолдол гарсан тул (cross-origin iframe + opacity:0
  // хослол дээрх нийтлэг WebKit асуудал) — харагдах товч дээрх ЖИНХЭНЭ
  // onClick-ээр Google-ийн албан ёсны prompt()-г шууд өдөөж, ижил
  // initialize()-д бүртгэгдсэн callback (handleGoogle)-руу орно.
  function triggerGoogleLogin() {
    if (window.google?.accounts?.id) {
      window.google.accounts.id.prompt(notif => {
        if (notif.isNotDisplayed?.() || notif.isSkippedMoment?.()) setErr(T.errGoogle);
      });
    } else {
      setErr(T.errGoogle);
    }
      ...
      <div ref={stageRef} className="login-stage" style={{
        ...
        <div className="login-card" style={{
          ...
          <div style={{ display:'flex' }}>
            <div className="sb" ref={googleBtnRef} onClick={triggerGoogleLogin}>
              ...
              <div style={{ position:'absolute', inset:0, opacity:0, overflow:'hidden', borderRadius:14, zIndex:1 }}>
                <GoogleLogin onSuccess={handleGoogle} onError={() => setErr(T.errGoogle)} width={googleBtnWidth} size="large" shape="rectangular" />
              </div>
            </div>
          </div>
